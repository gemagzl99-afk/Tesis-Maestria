Se descargaron los datos obtenidos de Novogene y se almacenaron en una carpeta "Datos Novogene". Posteriormente, se verificó la integridad de los archivos mediante el archivo MD5.txt, el cual contiene códigos conocidos como hasehes
MD5 para confirmar que los archivos no sufrieron daños ni modificaciones durante la transferencia. Cada archivo original posee una "huella digital" única generada mediante el algoritmo MD5; por lo tanto, cualquier cambio en el archivo,
incluso de solo un byte, produce una modificación en dicho código. Para realizar esta verificación se ejecutó el siguiente comando.

```
md5sum -c MD5.txt
```

Una vez comprobada la integridad de los datos, se generaron reportes de control de calidad usando FastQC. 

```
fastqc *.fq.gz -o fastqc_results
```

Este análisis evaluó todos los archivos FASTQ comprimidos (.fq.gz) presentes en la carpeta de trabajo y generó los archivos de salida *_fastqc.html y *_fastqc.zip, correspondientes a las muestras AWT, BWT3, CMUT y DMUT2.

Limpieza de datos con Fastp.
Elimina adaptadores, secuencias duplicadas y filtra lecturas de baja calidad. Script con editor Vim.
...
Ejecutar MultiQC de los resultados fastp. Script usado en vim.
...
multiqc /LUSTRE/bioinformatica_data/virolab/gemag/FastClean/fastQC/*.zip -o /LUSTRE/bioinformatica_data/virolab/gemag/FastClean/fastQC --data-format json --export

/LUSTRE/bioinformatica_data/virolab/gemag/DatosNovogene/01.RawData/fastqc_results

```

Descarga y transferencia del genoma de referencia.
Descargar el genoma completo de Sinorhizobium fredii NGR234 desde la base de datos del NCBI en formato, Download Package. Crear una carpeta (GenomaFredii).
Posteriormente, pasar el genoma de la PC a la terminal.
...
# --- CONFIGURACIÓN DE SOFTWARE (USANDO CONDA) ---

# 1. Carga el módulo base de Conda
module load conda-2023

# 2. AGREGA LA INICIALIZACIÓN DE CONDA 
# Carga las funciones necesarias para que 'conda activate' funcione en scripts
source $(conda info --base)/etc/profile.d/conda.sh

# 3. ACTIVA TU ENTORNO PERSONAL
conda activate hisat2_env

# Si Bowtie2, Jellyfish, Salmon No están en el entorno Conda, mantener rutas:
...
# 2. Ejecuta el comando Conda completo para activar el entorno
# Esto hace que el entorno esté disponible para la ejecución.


# --- Definición de Rutas (Verificadas) ---
...
# --- 2. Bucle para Mapeo (Alineamiento) y Ordenamiento ---
...
echo "Iniciando pre-procesamiento y eliminación de duplicados..."
    ...
    # Nombres de archivos intermedios y de salida
    ...
    # 2. Arreglar mates y añadir etiquetas MC/MD
    echo "2. Arreglando mates (fixmate): $FIXMATE_BAM"
    $SAMTOOLS fixmate -m -@ 12 "$NAME_SORTED" "$FIXMATE_BAM"

    # 3. Reordenar por posición (necesario para markdup y featureCounts)
    ...
    # 5. Indexación del nuevo BAM deduplicado
    echo "5. Indexando $DEDUP_BAM..."
    $SAMTOOLS index "$DEDUP_BAM"
# 6. Limpieza de archivos intermedios para ahorrar espacio
...
Verificar los archivos de salida y comparar el tamaño de lecturas.
...
Descarga y preparación del archivo de anotación (GFF). Para saber exactamente dónde están los genes en el genoma S. fredii.
...
# Agrega la carpeta 'bin' del entorno Conda al PATH del trabajo SLURM
export PATH="$CONDA_ENV_BASE/bin:$PATH"

# 3. VERIFICACIÓN DE ANOTACIÓN Y DATOS
# RUTA CORRECTA A TU ARCHIVO GFF/GFF3
